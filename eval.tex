...
%
To reduce the preparation effort for all three presentation tool conditions, we separated each slide into foreground and background elements, which are shown in our supplemental materials \wil{(are we doing that?)}. Moreover, for the \textit{BaselinPPT} condition, we further segmented the foreground into meaningful elements (e.g., arrows, labels, individual drawn objects) to make it easier for users to author fine-grained animations. 
%
